...
# Step 5: Decision and Output Generation (Simplified)
def generate_response(query: str, chunks: List[str], embeddings: np.ndarray = None, 
                     ...
                     llm_model: str = "anthropic/claude-3-haiku") -> str:
    ...
    # Import LLM prompt structure from app.py
...
# Interactive Q&A Function (Simplified)
async def interactive_qa_session_async(chunks: List[str], embeddings: np.ndarray, 
    ...
    while True:
        try:
            ...
            def sync_generate():
                return generate_response(user_query, chunks, embeddings, index, model_st)
            
            loop = asyncio.get_running_loop()
            response = await loop.run_in_executor(THREAD_POOL, sync_generate)
...
async def startup_event():
    ...
    try:
        ...
        loop = asyncio.get_running_loop()
        tasks.append(loop.run_in_executor(THREAD_POOL, load_sentence_transformer))
        tasks.append(loop.run_in_executor(THREAD_POOL, load_ner_pipeline))
...
async def hackrx_run(
    ...
    try:
        ...
        if cached_doc:
            ...
            loop = asyncio.get_running_loop()
            chunks, embeddings, index, model_st = await loop.run_in_executor(
                THREAD_POOL, process_document
            )
            ...
            asyncio.create_task(loop.run_in_executor(THREAD_POOL, cache_task))

        # Process questions concurrently for maximum speed
        async def process_question(question: str, q_index: int) -> str:
            try:
                logger.info(f"🤔 Processing Q{q_index+1}: {question[:50]}...")
                
                # Run the sync generate_response in thread pool
                def sync_generate():
                    return generate_response(question, chunks, embeddings, index, model_st)
                ...
                try:
                    ...
                    return result.get('justification', response)
                except json.JSONDecodeError:
                    return response
                    
            except Exception as e:
                logger.error(f"Error processing question {q_index+1}: {str(e)}")
        ...
        async def process_with_semaphore(q, i):
            async with semaphore:
                return await process_question(q, i)
